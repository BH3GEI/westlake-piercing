...
void ImageWriter::FixupClass(mirror::Class* orig, mirror::Class* copy) {
  // [DAYU600-IW-LOOKUP] diagnose whether MethodHandles$Lookup's fixed vtable survives image write
  std::string iw_desc_storage;
  const char* iw_desc = (orig != nullptr) ? orig->GetDescriptor(&iw_desc_storage) : nullptr;
  if (iw_desc != nullptr && strcmp(iw_desc, "Ljava/lang/invoke/MethodHandles$Lookup;") == 0) {
    static int iwl = 0;
    if (iwl < 3) { iwl++;
      int32_t vl = orig->GetVTableLength();
      int32_t fvh = -1, urvh = -1;
      ArtMethod* fvh_m = nullptr;
      for (int32_t vi = 0; vi < vl; vi++) {
        ArtMethod* m = orig->GetVTableEntry(vi, target_ptr_size_);
        if (m != nullptr && m->GetName() != nullptr) {
          if (strcmp(m->GetName(), "findVarHandle") == 0) { fvh = vi; fvh_m = m; }
          if (strcmp(m->GetName(), "unreflectVarHandle") == 0) urvh = vi;
        }
      }
      fprintf(stderr, "[IW-LOOKUP] orig vtable_len=%d findVarHandle@%d(idx=%d reloc=%d inboot=%d) "
              "unreflectVarHandle@%d\n",
              vl, fvh,
              fvh_m != nullptr ? (int)fvh_m->GetMethodIndex() : -1,
              fvh_m != nullptr ? (NativeRelocationAssigned(fvh_m) ? 1 : 0) : -1,
              fvh_m != nullptr ? (IsInBootImage(fvh_m) ? 1 : 0) : -1,
              urvh);
      fflush(stderr);
    }
  }
