...
class Dex2Oat final {
 ...
 private:
  ...
  void LogCompletionTime() {
    ...
    // [DAYU600] Skip driver_->GetMemoryUsageString(): on this arm64 on-device standalone build it
    // dumps arena MemStats through a std::ostream that SIGSEGVs at shutdown (ostream::sentry, bad
    // rdbuf) — purely cosmetic completion-time stats, but it crashes AFTER the (already finalized)
    // boot image is written+closed, turning a successful run into exit 139. Drop the stats string.
