...
ArtMethod* FindMethodToCall(Thread* self,
  ...
  if (type != kStatic && UNLIKELY((*this_object) == nullptr)) {
    if (UNLIKELY(IsStringInit(inst, caller))) {
      ...
      { static int dayu_nr = 0; if (false && dayu_nr < 20) { dayu_nr++;
        fprintf(stderr, "[DAYU600-NULLRECV] type=%d midx=%u caller=%s inst=[%s]\n",
                static_cast<int>(type), method_idx,
                caller != nullptr ? caller->PrettyMethod().c_str() : "?",
                caller != nullptr ? inst.DumpString(caller->GetDexFile()).c_str() : "?");
        fflush(stderr);
      }}
