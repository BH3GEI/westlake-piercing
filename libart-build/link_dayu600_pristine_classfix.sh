...
# Relink the known-booting A2OH bionic ARM64 object cache after replacing only
# runtime/mirror/class.o with the class-status no-op fix object. A full rebuild
# currently SIGBUSes during native registration on DAYU600, while this narrow
# relink reached DynTest.main() successfully.

ART_ROOT="${ART_ROOT:-/home/dspfac/art-pristine-classfix}"
NDK_ROOT="${NDK_ROOT:-/home/dspfac/android-sdk/ndk/25.2.9519653}"
B="${B:-build-bionic-arm64}"
...
"$CXX" -o "$B/bin/dalvikvm-pristine-plus-classfix" \
...
ls -lh "$B/bin/dalvikvm-pristine-plus-classfix"
file "$B/bin/dalvikvm-pristine-plus-classfix"
sha256sum "$B/bin/dalvikvm-pristine-plus-classfix"
