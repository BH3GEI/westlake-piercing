param(
    ...
    [ValidateSet('full', 'support', 'preferences')]
    [string]$Flavor = 'full'
)
...
$sources = @(Get-ChildItem -Path $fixture -Recurse -Filter '*.java' |
    Where-Object {
        if ($Flavor -eq 'preferences') {
            return $_.Name -eq 'e.java'
        }
        if ($Flavor -eq 'support') {
            return $_.Name -ne 'MainActivity.java'
        }
        return $true
    } |
    ForEach-Object { $_.FullName })
& $javac --release 8 -cp $androidJar -d $classes @sources
...
$suffix = if ($Flavor -eq 'full') { 'overlay' } else { "$Flavor-overlay" }
$output = Join-Path $out "dayu600-game-$suffix-stub.dex"
Copy-Item -Force (Join-Path $dexOut 'classes.dex') $output
