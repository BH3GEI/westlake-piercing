...
/// Source-over into a transparent OHOS overlay framebuffer.
#[inline]
fn blend_px(fb: &mut [u8], x: i32, y: i32, r: u32, g: u32, b: u32, a: u32) {
    ...
    let ia = 255 - a;
    let da = fb[o + 3] as u32;
    let oa = a + (da * ia + 127) / 255;
    let mix = |s: u32, d: u8| {
        let premul = s * a + (d as u32 * da * ia + 127) / 255;
        ((premul + oa / 2) / oa.max(1)) as u8
    };
    fb[o] = mix(r, fb[o]);
    fb[o + 1] = mix(g, fb[o + 1]);
    fb[o + 2] = mix(b, fb[o + 2]);
...
/// transparent first so GLES can composite it over Pocket3D.
