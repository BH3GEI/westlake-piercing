...
if ! "$HDC" -t "$SERIAL" shell "rm -f '$REMOTE_NATIVE' '$REMOTE_EARLY' '$REMOTE_PROBE' /data/local/tmp/w001-ckApp.txt /data/local/tmp/w001-ckFw.txt /data/local/tmp/w001-abA.txt /data/local/tmp/w001-abB.txt /data/local/tmp/w001-abAex.txt /data/local/tmp/w001-failex.txt /data/local/tmp/w001-failmsg.txt /data/local/tmp/w001-trace.txt" >"$OUT" 2>&1; then
  echo "log=$OUT"
...
if ! "$HDC" -t "$SERIAL" shell "echo __W001_NATIVE__; cat '$REMOTE_NATIVE' 2>/dev/null; echo __W001_EARLY__; cat '$REMOTE_EARLY' 2>/dev/null; echo __W001_PROBE__; cat '$REMOTE_PROBE' 2>/dev/null; echo __W001_TRACE__; cat /data/local/tmp/w001-trace.txt 2>/dev/null; echo __W001_CK__; cat /data/local/tmp/w001-ckApp.txt /data/local/tmp/w001-ckFw.txt 2>/dev/null; echo __W001_AB__; cat /data/local/tmp/w001-abA.txt /data/local/tmp/w001-abB.txt 2>/dev/null; echo __W001_EX__; cat /data/local/tmp/w001-abAex.txt /data/local/tmp/w001-failex.txt /data/local/tmp/w001-failmsg.txt 2>/dev/null" >>"$OUT" 2>&1; then
