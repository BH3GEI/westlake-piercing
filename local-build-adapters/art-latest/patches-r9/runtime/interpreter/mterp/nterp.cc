...
void CheckNterpAsmConstants() {
  // Stubbed for standalone dex2oat build: nterp asm handlers are not included,
  // and nterp is disabled for AOT compiler (CanRuntimeUseNterp returns false).
  // Skip the handler size check.
}
...
extern "C" size_t NterpGetMethod(Thread* self, ArtMethod* caller, const uint16_t* dex_pc_ptr)
  ...
  if (invoke_type == kInterface) {
    ...
    // [DAYU600-NTERP-VT] diagnose the findVarHandle->unreflectVarHandle vtable mis-dispatch
    {
      const char* rn = resolved_method->GetName();
      if (false && rn != nullptr && (strcmp(rn, "findVarHandle") == 0 || strcmp(rn, "unreflectVarHandle") == 0)) {
        static int nvt = 0;
        if (nvt < 8) { nvt++;
          PointerSize ps = Runtime::Current()->GetClassLinker()->GetImagePointerSize();
          uint32_t vidx = resolved_method->GetMethodIndex();
          ObjPtr<mirror::Class> dc = resolved_method->GetDeclaringClass();
          int32_t vlen = (dc != nullptr) ? dc->GetVTableLength() : -1;
          ArtMethod* atslot = (dc != nullptr && (int32_t)vidx < vlen)
              ? dc->GetVTableEntry(vidx, ps) : nullptr;
          fprintf(stderr, "[NTERP-VT] method_idx=%u resolved=%s vtable_idx=%u decl=%s vtable_len=%d at_slot=%s flags{direct=%d final=%d copied=%d miranda=%d public=%d} num_virt=%u num_direct=%u\n",
                  method_index, resolved_method->PrettyMethod().c_str(), vidx,
                  dc != nullptr ? dc->PrettyDescriptor().c_str() : "?", vlen,
                  atslot != nullptr ? atslot->PrettyMethod().c_str() : "null",
                  resolved_method->IsDirect()?1:0, resolved_method->IsFinal()?1:0,
                  resolved_method->IsCopied()?1:0, resolved_method->IsMiranda()?1:0,
                  resolved_method->IsPublic()?1:0,
                  dc != nullptr ? dc->NumVirtualMethods() : 0u,
                  dc != nullptr ? dc->NumDirectMethods() : 0u);
          if (dc != nullptr && nvt == 1) {
            for (int32_t vi = 0; vi < vlen; vi++) {
              ArtMethod* ve = dc->GetVTableEntry(vi, ps);
              fprintf(stderr, "   vtable[%d] = %s\n", vi, ve != nullptr ? ve->PrettyMethod().c_str() : "null");
            }
          }
          fflush(stderr);
        }
