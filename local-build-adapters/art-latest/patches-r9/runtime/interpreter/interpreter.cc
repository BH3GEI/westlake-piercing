...
static void InterpreterJni(Thread* self,
                                    ...
                                    // dispatch below must call THIS, not re-read the (stale) JNI entry.
  {
    ...
    if (jni_entry == dlsym_stub || jni_entry == dlsym_critical_stub || jni_entry == nullptr ||
        wl_is_xmlblock) {
      // RegisterNatives may have staged an @CriticalNative pointer until visible
      // initialization. Consult ART's registered-native lifecycle before symbol lookup.
      ClassLinker* class_linker = Runtime::Current()->GetClassLinker();
      const void* native_code = wl_is_xmlblock
          ? nullptr
          : class_linker->GetRegisteredNative(self, method);
