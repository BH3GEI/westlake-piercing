...
static const void* PFCutCodeForQuickTrampoline(ArtMethod* method, const char* site)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  const void* code = method->GetEntryPointFromQuickCompiledCode();
  static constexpr uintptr_t kPFCutStaleQuickEntry = 0xfffffffffffffb17ULL;
  if (UNLIKELY(reinterpret_cast<uintptr_t>(code) == kPFCutStaleQuickEntry)) {
    ...
    if (repair_count < 80) {
      repair_count++;
      fprintf(stderr,
              "[PFCUT] quick trampoline stale repair site=%s native=%d %s\n",
              site,
              method->IsNative() ? 1 : 0,
...
extern "C" uint64_t artQuickToInterpreterBridge(ArtMethod* method, Thread* self, ArtMethod** sp)
  ...
  if (UNLIKELY(method->IsNative() &&
               ...
               method->GetShortyView() == "L")) {
    const bool trace_tz = WestlakeTraceTimeZoneBridge();
    if (trace_tz) {
      fprintf(stderr,
              "[WESTLAKE-TZ-QUICK] direct quick bridge object method=%s jni=%p\n",
              method->PrettyMethod().c_str(),
              const_cast<void*>(method->GetEntryPointFromJni()));
      fflush(stderr);
    }
    void* result_obj = Westlake_TimeZone_getDefaultObjectForQuick();
    if (trace_tz) {
      fprintf(stderr,
              "[WESTLAKE-TZ-QUICK] direct quick bridge result object=%p\n",
              result_obj);
      fflush(stderr);
    }
    return reinterpret_cast<uintptr_t>(result_obj);
  }
 ...
 */
// No-op body for an UNREGISTERED native we choose to survive rather than abort on. Takes no
// declared args, so it is safe to invoke under either the normal or @CriticalNative ABI
// (AArch64 caller-cleanup ignores the extra register/stack args). Used for framework-16 graphics
// natives the OHOS adapter libhwui doesn't bind (e.g. Paint.nSetFlags) so View inflate proceeds.
...
extern "C" const void* artQuickGenericJniTrampoline(Thread* self,
  ...
  // Boot-image native entrypoints can be stale in standalone/portable mode.
  // Resolve them before switching the thread to native state; otherwise the
  // assembly trampoline will branch directly to the stale JNI address.
  void const* nativeCode = called->GetEntryPointFromJni();
  static constexpr uintptr_t kPFCutStaleNativeEntry = 0xfffffffffffffb17ULL;
  if (UNLIKELY(reinterpret_cast<uintptr_t>(nativeCode) == kPFCutStaleNativeEntry)) {
    called->SetEntryPointFromJniPtrSize(nullptr, kRuntimePointerSize);
    nativeCode = nullptr;
  }
  if (UNLIKELY(nativeCode == nullptr ||
    ...
    if (repair_count < 80) {
      repair_count++;
      fprintf(stderr,
              "[PFCUT] GenericJNI resolved stale/unset native entry %s -> %p\n",
...
static TwoWordReturn artInvokeCommon(uint32_t method_idx,
  ...
  DCHECK(!self->IsExceptionPending());
  const void* code = PFCutCodeForQuickTrampoline(method, "artInvokeCommon");
...
extern "C" TwoWordReturn artInvokeInterfaceTrampoline(ArtMethod* interface_method,
  ...
  if (UNLIKELY(resolve_method)) {
    ...
    if (method != nullptr) {
      const void* code = PFCutCodeForQuickTrampoline(method, "imt-conflict");
  ...
  const void* code = PFCutCodeForQuickTrampoline(method, "interface-trampoline");
