...
// Boot classpath classes (AtomicInteger, ThreadLocal, ConcurrentHashMap, etc.)
// need to reflect on their own private fields via VarHandle/findVarHandle.
// Without this, the VarHandle -> AtomicInteger -> System cascade fails.
template <typename T>
bool ShouldDenyAccessToMember([[maybe_unused]] T* member,
                              [[maybe_unused]] const std::function<AccessContext()>& fn_get_access_context,
                              [[maybe_unused]] AccessMethod access_method) {
  return false;
}
