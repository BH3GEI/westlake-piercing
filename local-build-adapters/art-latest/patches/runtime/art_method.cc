...
void ArtMethod::Invoke(Thread* self, uint32_t* args, uint32_t args_size, JValue* result,
  ...
  if (UNLIKELY(!runtime->IsStarted() || force_interpreter_path ||
    ...
    if (LIKELY(have_quick_code)) {
      ...
      // No compiled code available. Fall back to the interpreter instead of skipping.
      // This is needed for dex2oat standalone builds where methods aren't compiled
      // but clinit must still run for class initialization.
      if (IsInvokable() && !IsNative()) {
        if (IsStatic()) {
          art::interpreter::EnterInterpreterFromInvoke(
              self, this, nullptr, args, result, /*stay_in_interpreter=*/ true);
        } else {
