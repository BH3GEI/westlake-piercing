...
bool ClassLinker::InitWithoutImage(std::vector<std::unique_ptr<const DexFile>> boot_class_path,
  ...
  if (java_lang_DexCache->GetObjectSize() != mirror::DexCache::InstanceSize()) {
    LOG(WARNING) << "DexCache size mismatch: DEX=" << java_lang_DexCache->GetObjectSize()
                 << " native=" << mirror::DexCache::InstanceSize() << " (cross-version, adjusting)";
    java_lang_DexCache->SetObjectSize(mirror::DexCache::InstanceSize());
  }
  ...
  if (dalvik_system_ClassExt->GetObjectSize() != mirror::ClassExt::InstanceSize()) {
    LOG(WARNING) << "ClassExt size mismatch: DEX=" << dalvik_system_ClassExt->GetObjectSize()
                 << " native=" << mirror::ClassExt::InstanceSize() << " (cross-version, adjusting)";
    dalvik_system_ClassExt->SetObjectSize(mirror::ClassExt::InstanceSize());
  }
...
void ClassLinker::RunRootClinits(Thread* self) {
  ...
  // (circular dependency through AtomicInteger/AtomicLong -> VarHandle -> Enum).
  {
    ...
    for (const char* desc : pre_init_classes) {
      ...
      if (false && strstr(desc, "VarHandle") != nullptr) {  // [DAYU600-UNSKIP-VH] wild-write fixed
      ...
      // [DAYU600-HEAP-SCAN] after each clinit, walk the heap and flag any object whose class-word
      // is out-of-heap (garbage) -> pinpoints the FIRST clinit that produces the wild-write corruption.
      {
        gc::Heap* dayu_heap = Runtime::Current()->GetHeap();
        size_t dayu_bad = 0;
        const char* dayu_cur = desc;
        dayu_heap->VisitObjects([&](mirror::Object* obj) REQUIRES_SHARED(Locks::mutator_lock_) {
          if (obj == nullptr) return;
          mirror::Class* c = obj->GetClass<kVerifyNone>();
          if (c == nullptr || dayu_heap->FindSpaceFromAddress(c) == nullptr) {
            if (dayu_bad < 4) {
              fprintf(stderr, "[DAYU600-HEAP-SCAN] CORRUPT obj=%p class=%p after=%s\n",
                      reinterpret_cast<void*>(obj), reinterpret_cast<void*>(c), dayu_cur);
            }
            dayu_bad++;
          }
        });
        if (dayu_bad > 0) {
          fprintf(stderr, "[DAYU600-HEAP-SCAN] >>> %zu corrupt object(s) FIRST seen after clinit of: %s\n",
                  dayu_bad, desc);
          fflush(stderr);
        }
      }
  ...
  if (!Runtime::Current()->GetHeap()->HasBootImageSpace()) {
    ...
    for (size_t i = 0; i < static_cast<size_t>(ClassRoot::kMax); ++i) {
      ...
      if (false && (cr == ClassRoot::kJavaLangInvokeVarHandle ||
          cr == ClassRoot::kJavaLangInvokeFieldVarHandle ||
          cr == ClassRoot::kJavaLangInvokeStaticFieldVarHandle)) {  // [DAYU600-UNSKIP-VH]
        continue;
      }
    ...
    if (self->IsExceptionPending()) {
      ...
      // Mark as initialized so dependent classes can proceed.
      {
        ...
        LOG(WARNING) << "Tolerating clinit failure for " << desc
                     << ": " << self->GetException()->Dump();
        self->ClearException();
        // [DAYU600-RT-LOOKUP2] definitive: when tolerating AtomicInteger, dump the RUNTIME loaded
        // MethodHandles$Lookup vtable to settle image-content (37 = image good, 11 = image persisted broken).
        if (false && desc != nullptr && strstr(desc, "AtomicInteger") != nullptr) {
          ObjPtr<mirror::Class> lk =
              GetClassRoot(ClassRoot::kJavaLangInvokeMethodHandlesLookup, this);
          LOG(WARNING) << "[RT-LOOKUP2] reached for " << desc << " lk_null=" << (lk == nullptr);
          if (lk != nullptr) {
            int32_t vl = lk->GetVTableLength();
            int32_t fvh = -1, urvh = -1, fvh_idx = -1;
            for (int32_t vi = 0; vi < vl; vi++) {
              ArtMethod* m = lk->GetVTableEntry(vi, image_pointer_size_);
              if (m != nullptr && m->GetName() != nullptr) {
                if (strcmp(m->GetName(), "findVarHandle") == 0) { fvh = vi; fvh_idx = m->GetMethodIndex(); }
                if (strcmp(m->GetName(), "unreflectVarHandle") == 0) urvh = vi;
              }
            }
            LOG(WARNING) << "[DAYU600-RT-LOOKUP2] runtime Lookup vtable_len=" << vl
                         << " findVarHandle@" << fvh << "(idx=" << fvh_idx << ")"
                         << " unreflectVarHandle@" << urvh;
          }
        }
        // [DAYU600-HEAP-SCAN2] scan after each tolerated clinit -> catch the FIRST corrupting class
        {
          gc::Heap* dh2 = Runtime::Current()->GetHeap();
          size_t db2 = 0;
          void* dsample = nullptr;
          dh2->VisitObjects([&](mirror::Object* o) REQUIRES_SHARED(Locks::mutator_lock_) {
            if (o == nullptr) return;
            mirror::Class* c = o->GetClass<kVerifyNone>();
            if (c == nullptr || dh2->FindSpaceFromAddress(c) == nullptr) {
              if (db2 == 0) dsample = reinterpret_cast<void*>(c);
              db2++;
            }
          });
          if (db2 > 0) {
            fprintf(stderr, "[DAYU600-HEAP-SCAN2] >>> %zu CORRUPT objects (sample class=%p) FIRST after tolerated clinit of: %s\n",
                    db2, dsample, desc);
            fflush(stderr);
          }
        }
        if (strcmp(desc, "Ljava/util/concurrent/ConcurrentHashMap;") == 0) {
          ObjPtr<mirror::Class> unsafe_class =
              FindSystemClass(self, "Ljdk/internal/misc/Unsafe;");
          if (self->IsExceptionPending()) {
            self->ClearException();
          }
          if (unsafe_class != nullptr) {
            ...
            Handle<mirror::Class> h_chm = hs.NewHandle(klass.Get());

            ArtField* theUnsafeField = nullptr;
            for (uint32_t i = 0; i < h_unsafe->NumStaticFields(); ++i) {
              ArtField* f = h_unsafe->GetStaticField(i);
              if (strcmp(f->GetName(), "theUnsafe") == 0) {
                theUnsafeField = f;
                break;
              }
            }
            ...
            for (uint32_t i = 0; i < h_chm->NumStaticFields(); ++i) {
              ArtField* f = h_chm->GetStaticField(i);
              if (strcmp(f->GetName(), "U") == 0) {
                uField = f;
                break;
              }
            }

            if (theUnsafeField != nullptr && uField != nullptr) {
              ObjPtr<mirror::Object> unsafeObj = theUnsafeField->GetObject(h_unsafe.Get());
              if (unsafeObj == nullptr) {
                ...
                if (unsafeObj != nullptr) {
                  theUnsafeField->SetObject<false>(h_unsafe.Get(), unsafeObj);
                  fprintf(stderr, "[CL] Recreated Unsafe.theUnsafe after CHM clinit failure\n");
                }
              }
              if (unsafeObj != nullptr) {
                uField->SetObject<false>(h_chm.Get(), unsafeObj);
                fprintf(stderr, "[CL] Repaired ConcurrentHashMap.U after clinit failure\n");
              }
            }
            ...
            for (int j = 0; j < 4; ++j) {
              if (fields[j] != nullptr) {
                for (uint32_t i = 0; i < h_chm->NumInstanceFields(); ++i) {
                  ArtField* f = h_chm->GetInstanceField(i);
                  if (strcmp(f->GetName(), inames[j]) == 0) {
                    fields[j]->SetLong<false>(h_chm.Get(), f->GetOffset().SizeValue());
                    break;
                  }
                }
              }
            ...
            for (uint32_t i = 0; i < h_chm->NumStaticFields(); ++i) {
              ArtField* f = h_chm->GetStaticField(i);
              if (strcmp(f->GetName(), "ABASE") == 0) {
                abaseF = f;
              } else if (strcmp(f->GetName(), "ASHIFT") == 0) {
                ashiftF = f;
              }
            }
            if (abaseF != nullptr && ashiftF != nullptr) {
              const int32_t abase = mirror::Array::DataOffset(
                  sizeof(mirror::HeapReference<mirror::Object>)).Int32Value();
              const int32_t scale = sizeof(mirror::HeapReference<mirror::Object>);
              int32_t ashift = 0;
              while ((1 << ashift) < scale) {
                ++ashift;
              }
              abaseF->SetInt<false>(h_chm.Get(), abase);
              ashiftF->SetInt<false>(h_chm.Get(), ashift);
              fprintf(stderr,
                      "[CL] Repaired ConcurrentHashMap ABASE=%d ASHIFT=%d after clinit failure\n",
                      abase,
                      ashift);
            }
            fflush(stderr);
        ...
        } else if (strcmp(desc, "Ldalvik/system/VMRuntime;") == 0) {
          StackHandleScope<5> hs(self);
          Handle<mirror::Class> h_vm = hs.NewHandle(klass.Get());

          ArtField* theOneF = nullptr;
          for (uint32_t i = 0; i < h_vm->NumStaticFields(); ++i) {
            ArtField* f = h_vm->GetStaticField(i);
            if (strcmp(f->GetName(), "THE_ONE") == 0) {
              theOneF = f;
              break;
            }
          }

          ObjPtr<mirror::Object> vm_obj =
              theOneF != nullptr ? theOneF->GetObject(h_vm.Get()) : nullptr;
          if (vm_obj == nullptr) {
            vm_obj = h_vm->AllocObject(self);
            if (self->IsExceptionPending()) {
              self->ClearException();
            }
          }
          Handle<mirror::Object> h_vm_obj = hs.NewHandle(vm_obj);
          if (theOneF != nullptr && h_vm_obj.Get() != nullptr) {
            theOneF->SetObject<false>(h_vm.Get(), h_vm_obj.Get());
          }

          ArtField* allocationCountF = nullptr;
          ArtField* disabledCompatChangesF = nullptr;
          ArtField* notifyNativeIntervalF = nullptr;
          ArtField* targetSdkVersionF = nullptr;
          for (uint32_t i = 0; i < h_vm->NumInstanceFields(); ++i) {
            ArtField* f = h_vm->GetInstanceField(i);
            const char* name = f->GetName();
            if (strcmp(name, "allocationCount") == 0) {
              allocationCountF = f;
            } else if (strcmp(name, "disabledCompatChanges") == 0) {
              disabledCompatChangesF = f;
            } else if (strcmp(name, "notifyNativeInterval") == 0) {
              notifyNativeIntervalF = f;
            } else if (strcmp(name, "targetSdkVersion") == 0) {
              targetSdkVersionF = f;
            }
          }

          if (h_vm_obj.Get() != nullptr && allocationCountF != nullptr) {
            ObjPtr<mirror::Class> ai_class =
                FindSystemClass(self, "Ljava/util/concurrent/atomic/AtomicInteger;");
            if (self->IsExceptionPending()) {
              self->ClearException();
            }
            if (ai_class != nullptr) {
              Handle<mirror::Class> h_ai = hs.NewHandle(ai_class);
              ObjPtr<mirror::Object> ai_obj = h_ai->AllocObject(self);
              if (self->IsExceptionPending()) {
                self->ClearException();
              }
              Handle<mirror::Object> h_ai_obj = hs.NewHandle(ai_obj);
              if (h_ai_obj.Get() != nullptr) {
                for (uint32_t i = 0; i < h_ai->NumInstanceFields(); ++i) {
                  ArtField* f = h_ai->GetInstanceField(i);
                  if (strcmp(f->GetName(), "value") == 0) {
                    f->SetInt<false>(h_ai_obj.Get(), 0);
                    break;
                  }
                }
                allocationCountF->SetObject<false>(h_vm_obj.Get(), h_ai_obj.Get());
              }
            }
          }

          if (h_vm_obj.Get() != nullptr && disabledCompatChangesF != nullptr) {
            Handle<mirror::LongArray> h_empty_longs =
                hs.NewHandle(mirror::LongArray::Alloc(self, 0));
            if (self->IsExceptionPending()) {
              self->ClearException();
            }
            if (h_empty_longs.Get() != nullptr) {
              disabledCompatChangesF->SetObject<false>(h_vm_obj.Get(), h_empty_longs.Get());
            }
          }
          if (h_vm_obj.Get() != nullptr && notifyNativeIntervalF != nullptr) {
            notifyNativeIntervalF->SetInt<false>(h_vm_obj.Get(), 384);
          }
          if (h_vm_obj.Get() != nullptr && targetSdkVersionF != nullptr) {
            targetSdkVersionF->SetInt<false>(h_vm_obj.Get(), 35);
          }
          fprintf(stderr, "[CL] Repaired VMRuntime.THE_ONE after clinit failure\n");
          fflush(stderr);
          if (self->IsExceptionPending()) {
            self->ClearException();
          }
...
static bool IsWestlakeDescriptorNeedingRelaxedCrossLoaderValidation(const std::string& desc) {
  return android::base::StartsWith(desc, "Lcom/digiplex/") ||
         android::base::StartsWith(desc, "Lcom/google/android/material/") ||
         android::base::StartsWith(desc, "Landroidx/activity/") ||
         android::base::StartsWith(desc, "Landroidx/appcompat/") ||
         android::base::StartsWith(desc, "Landroidx/core/") ||
         android::base::StartsWith(desc, "Landroidx/fragment/") ||
         android::base::StartsWith(desc, "Landroidx/lifecycle/") ||
         android::base::StartsWith(desc, "Landroidx/savedstate/") ||
         android::base::StartsWith(desc, "Lkotlin/") ||
...
size_t ClassLinker::LinkMethodsHelper<kPointerSize>::AssignVTableIndexes(
  ...
  // [DAYU600-VTABLE-FIX] bit_vector_buffer_ptr comes from un-zeroed alloca stack memory (BitVector
  // does NOT zero external storage). On arm64 on-device boot-image creation the stack holds garbage
  // (e.g. 0xFE from ClobberOldMethods), so initialized_methods starts with bits SET -> every declared
  // virtual method is falsely treated as already-overriding -> vtable truncated to super length
  // (MethodHandles$Lookup: 27 virtuals all dropped, findVarHandle/unreflectVarHandle collide at idx 0
  // -> invoke-virtual mis-dispatch -> clone-on-Class -> CloneNotSupported). Zero it before use.
  // [DAYU600-VTABLE-FIX v3] Zero the bit-vector storage BEFORE constructing/using it. The buffer is
  // un-zeroed alloca stack memory; reading it uninitialized is UB, and -O2 exploits that to ELIMINATE
  // a plain std::fill_n/memset (and ClearAllBits didn't help). Writing through a VOLATILE pointer has
  // observable side effects the optimizer must not drop, guaranteeing the storage is really zeroed.
  // On arm64 on-device boot-image creation the stack holds garbage (0xFE from ClobberOldMethods), so
  // without this every declared virtual method is falsely flagged already-overriding -> vtable
  // truncated to super length (MethodHandles$Lookup: 27 virtuals dropped, findVarHandle collides with
  // unreflectVarHandle at idx 0 -> clone-on-Class -> CloneNotSupported).
  {
    volatile uint32_t* dayu_vbuf = bit_vector_buffer_ptr;
    for (size_t dayu_wi = 0; dayu_wi < bit_vector_size; ++dayu_wi) { dayu_vbuf[dayu_wi] = 0u; }
  }
  ...
  // [DAYU600-AVT] diagnose MethodHandles$Lookup vtable truncation
  {
    static int davt = 0;
    if (false && davt < 4 && klass->DescriptorEquals("Ljava/lang/invoke/MethodHandles$Lookup;")) { davt++;
      size_t overr = 0;
      for (size_t i = 0; i < num_virtual_methods; ++i) { if (initialized_methods.IsBitSet(i)) overr++; }
      fprintf(stderr, "[AVT] Lookup num_virtual_methods=%zu super_vtable_length=%zu final_vtable_length=%zu overriding=%zu\n",
              num_virtual_methods, super_vtable_length, vtable_length, overr);
      fflush(stderr);
    }
  }
  ...
  if (is_static) {
    ...
    *class_size = size;
    ...
    if (!klass->IsVariableSize()) {
      ...
      if (previous_size != 0 && previous_size != size) {
        // Relaxed for standalone dex2oat with cross-version core JARs.
        LOG(WARNING) << "Object size mismatch for " << klass->GetDescriptor(&temp)
                     << " (previous=" << previous_size << " new=" << size << ")";
      }
