...
// libcore.io.Memory.unsafeBulkGet(Object dst, int dstOffset, int byteCount, byte[] src,
//   int srcOffset, int sizeofT, boolean swap) — bulk copy from a byte[] into a primitive
//   dst array (element size sizeofT bytes), optionally byte-swapping each element. Used by
//   HeapByteBuffer bulk gets (e.g. ByteBufferAsCharBuffer.get) when parsing ICU .res data.
//   Not present in this AOSP-15 snapshot; added for the DAYU600 port.
static void Memory_unsafeBulkGet(JNIEnv* env, jclass, jobject dstObject, jint dstOffset,
                                 ...
                                 jint sizeofT, jboolean swap) {
  jbyte* srcBytes = reinterpret_cast<jbyte*>(env->GetPrimitiveArrayCritical(srcArray, nullptr));
  if (srcBytes == nullptr) {
    return;
  }
  jarray dstArray = reinterpret_cast<jarray>(dstObject);
  jbyte* dstBytes = reinterpret_cast<jbyte*>(env->GetPrimitiveArrayCritical(dstArray, nullptr));
  if (dstBytes == nullptr) {
    env->ReleasePrimitiveArrayCritical(srcArray, srcBytes, JNI_ABORT);
    return;
  }
  jbyte* dst = dstBytes + static_cast<size_t>(dstOffset) * static_cast<size_t>(sizeofT);
  const jbyte* src = srcBytes + srcOffset;
  if (swap && sizeofT > 1) {
    size_t count = static_cast<size_t>(byteCount) / static_cast<size_t>(sizeofT);
    if (sizeofT == 2) {
      swapShorts(reinterpret_cast<jshort*>(dst), reinterpret_cast<const jshort*>(src), count);
    } else if (sizeofT == 4) {
      swapInts(reinterpret_cast<jint*>(dst), reinterpret_cast<const jint*>(src), count);
    } else if (sizeofT == 8) {
      swapLongs(reinterpret_cast<jlong*>(dst), reinterpret_cast<const jlong*>(src), count);
    } else {
