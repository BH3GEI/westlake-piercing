...
static jclass Proxy_generateProxy(JNIEnv* env, jclass, jstring name, jobjectArray interfaces,
  ...
  ObjPtr<mirror::Class> result = class_linker->CreateProxyClass(
      soa, name, interfaces, loader, methods, throws);
  if (result == nullptr) {
    fprintf(stderr, "[WL-PROXY] CreateProxyClass returned null; exceptionPending=%d\n",
            soa.Self()->IsExceptionPending());
    if (soa.Self()->IsExceptionPending()) {
      fprintf(stderr, "[WL-PROXY] pending exception: %s\n",
              soa.Self()->GetException()->Dump().c_str());
    }
    fflush(stderr);
  }
