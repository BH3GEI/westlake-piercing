...
static jobject DexFile_openInMemoryDexFilesNative(JNIEnv* env,
  ...
  // [DAYU600 MULTIDEX DIAG] How many dex byte-buffers did libcore hand us? If a multi-dex APK
  // arrives here with buffers_length==1, the loss is upstream in libcore's zip extraction.
  LOG(ERROR) << "WESTLAKE_INMEM buffers_length=" << buffers_length;
  ...
  std::vector<std::unique_ptr<const DexFile>> dex_files =
      Runtime::Current()->GetOatFileManager().OpenDexFilesFromOat(std::move(dex_mem_maps),
                                                                  ...
                                                                  /*out*/ &oat_file,
                                                                  /*out*/ &error_msgs);
  LOG(ERROR) << "WESTLAKE_INMEM out ndex=" << dex_files.size()
             << " (from buffers_length=" << buffers_length << ")";
  for (size_t di = 0; di < dex_files.size(); ++di) {
    LOG(ERROR) << "WESTLAKE_INMEM  dex[" << di << "]=" << dex_files[di]->GetLocation();
  }
...
static jobject DexFile_openDexFileNative(JNIEnv* env,
  ...
  // [DAYU600 MULTIDEX DIAG] Unconditional entry marker — proves whether libcore reaches this
  // native opener at all for a given path (e.g. the app APK / an extracted classesN.dex).
  LOG(ERROR) << "WESTLAKE_OPENFILE enter path=" << sourceName.c_str();
  ...
  std::vector<std::unique_ptr<const DexFile>> dex_files =
      Runtime::Current()->GetOatFileManager().OpenDexFilesFromOat(sourceName.c_str(),
                                                                  ...
                                                                  /*out*/ &oat_file,
                                                                  /*out*/ &error_msgs);
  // [DAYU600 MULTIDEX DIAG] Log how many dex files the loader returned for this APK path,
  // to bisect whether only classes.dex is exposed at the loader boundary.
  LOG(ERROR) << "WESTLAKE_MULTIDEX path=" << sourceName.c_str()
             << " ndex=" << dex_files.size()
             << " nerr=" << error_msgs.size();
  for (size_t di = 0; di < dex_files.size(); ++di) {
    LOG(ERROR) << "WESTLAKE_MULTIDEX  dex[" << di << "]=" << dex_files[di]->GetLocation();
  }
  for (size_t ei = 0; ei < error_msgs.size(); ++ei) {
    LOG(ERROR) << "WESTLAKE_MULTIDEX  err[" << ei << "]=" << error_msgs[ei];
  }
  return CreateCookieFromOatFileManagerResult(env, dex_files, oat_file, error_msgs);
}
...
static void DexFile_verifyInBackgroundNative(JNIEnv* env,
                                             ...
                                             jobject cookie,
                                             jobject class_loader) {
  // [DAYU600] Background dex verification spawns a verifier thread that aborts in this
  // standalone runtime (multi-dex APK PathClassLoader triggers it). It is an optimization
  // only — classes verify on first use anyway under -Xverify:none/interpreter — skip it.
  (void)env; (void)cookie; (void)class_loader;
}
