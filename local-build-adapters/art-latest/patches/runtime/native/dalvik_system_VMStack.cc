...
// Returns the class of the caller's caller's caller: this native frame (0),
// Reflection.getCallerClass (1), the @CallerSensitive method that called it (2),
// then the real caller we want (3). NthCallerVisitor counts only real Java frames
// — it already skips the runtime/bridge frames the standalone interpreter inserts
// — so a fixed depth of 3 is exact with JIT/inlining off, matching stock AOSP.
//
// This deliberately restores the stock NthCallerVisitor(3) implementation. An
// earlier local variant walked by declaring-class name, skipping VMStack /
// MethodHandles / sun.reflect.Reflection and returning the first other frame. That
// variant stopped on jdk.internal.reflect.Reflection (which it did not list to
// skip) and, even with it listed, would return the @CallerSensitive method itself
// rather than its caller — breaking AtomicReferenceFieldUpdater.newUpdater (and the
// Integer/Long variants) for every private volatile field: the access check saw the
// wrong caller, newUpdater threw IllegalAccessException, the field's <clinit> failed,
// and the null updater then NPE'd at first use.
