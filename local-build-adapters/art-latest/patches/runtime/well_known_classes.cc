...
static ObjPtr<mirror::Class> FindSystemClass(ClassLinker* class_linker,
  ...
  if (klass == nullptr) {
    // Standalone dex2oat: tolerate missing system classes, clear exception
    LOG(WARNING) << "WellKnownClasses: Couldn't find system class: " << descriptor;
...
static jclass CacheClass(JNIEnv* env, const char* jni_class_name) {
  ...
  if (c.get() == nullptr) {
    LOG(WARNING) << "WellKnownClasses: Couldn't find class: " << jni_class_name;
    env->ExceptionClear();
    return nullptr;
  }
...
static ArtField* CacheField(ObjPtr<mirror::Class> klass,
  ...
  if (UNLIKELY(field == nullptr)) {
    LOG(WARNING) << "WellKnownClasses: Couldn't find " << (is_static ? "static" : "instance")
                 << " field \"" << name << "\" with signature \"" << signature << "\"";
  }
  return field;
}
...
void WellKnownClasses::InitStringInit(ObjPtr<mirror::Class> string_class,
  ...
  auto find_method = [p_size](ObjPtr<mirror::Class> klass,
                              const char* name,
                              const char* sig,
                              bool expext_static) REQUIRES_SHARED(Locks::mutator_lock_) -> ArtMethod* {
    if (klass == nullptr) return nullptr;
    ArtMethod* ret = klass->FindClassMethod(name, sig, p_size);
    if (ret == nullptr) {
      LOG(WARNING) << "WellKnownClasses: Couldn't find string init method " << name << sig;
      return nullptr;
    }
    if (expext_static != ret->IsStatic()) {
      LOG(WARNING) << "WellKnownClasses: Method " << name << sig << " static mismatch";
      return nullptr;
    }
    return ret;
  };
