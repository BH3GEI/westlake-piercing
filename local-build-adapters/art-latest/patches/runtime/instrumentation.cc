...
bool Instrumentation::InterpretOnly(ArtMethod* method) REQUIRES_SHARED(Locks::mutator_lock_) {
  // PATCH: Don't skip native methods — their AOT stubs have stale addresses.
  // All native methods go through InterpreterJni in standalone mode.
  return InterpretOnly() || IsDeoptimized(method);
}

static bool CanUseAotCode(const void* quick_code)
    REQUIRES_SHARED(Locks::mutator_lock_) {
  // PATCH: Never use AOT code — it has stale addresses from dex2oat.
  // All methods go through interpreter in standalone mode.
  return false;
