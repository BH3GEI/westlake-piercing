...
inline bool ObjectArray<T>::CheckAssignable(ObjPtr<T> object) {
  if (object != nullptr) {
    ...
    if (UNLIKELY(!object->InstanceOf(element_class))) {
      // PF-noice-002 (2026-05-04): Westlake's standalone-dalvikvm class
      // identity has duplicates for boot classes (two String, two
      // CoderResult etc. with same descriptor); InstanceOf uses Class
      // identity, which fails. Generalize the descriptor fallback —
      // if the destination component descriptor matches the value
      // descriptor, allow the store. Models PFCutArraycopyElementAssignable
      // (interpreter_common.cc:1867). Empirically eliminates the
      // ICU/CharsetProvider/Crypto/etc clinit cascade.
      ObjPtr<Class> object_class = object->template GetClass<kVerifyFlags>();
      if (UNLIKELY(element_class != nullptr && object_class != nullptr)) {
        std::string element_desc;
        std::string object_desc;
        const char* element_d = element_class->GetDescriptor(&element_desc);
        const char* object_d = object_class->GetDescriptor(&object_desc);
        if (element_d != nullptr && object_d != nullptr &&
            strcmp(element_d, object_d) == 0) {
          return true;
        }
      }
