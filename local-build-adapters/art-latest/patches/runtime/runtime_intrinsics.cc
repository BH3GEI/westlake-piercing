...
ArtMethod* FindIntrinsicMethod(Thread* self,
  ...
  if (cls == nullptr) {
    // Standalone dex2oat: tolerate missing intrinsic classes
    LOG(WARNING) << "Could not find class of intrinsic " << class_name;
  ...
  if (method == nullptr || method->GetDeclaringClass() != cls) {
    LOG(WARNING) << "Could not find method of intrinsic "
...
bool InitializeIntrinsic(Thread* self,
  ...
  if (method == nullptr) return false;  // Tolerate missing intrinsics

  if (method->GetInvokeType() != invoke_type) return false;
