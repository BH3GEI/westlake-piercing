...
void Thread::AssertNoPendingExceptionForNewException(const char* msg) const {
  if (UNLIKELY(IsExceptionPending())) {
    ScopedObjectAccess soa(Thread::Current());
    LOG(WARNING) << "Throwing new exception '" << msg
        << "' with unexpected pending exception in standalone mode; clearing: "
        << GetException()->Dump();
    const_cast<Thread*>(this)->ClearException();
  }
}
...
void Thread::ThrowNewWrappedException(const char* exception_class_descriptor,
  ...
  if (!runtime->IsAotCompiler() && UNLIKELY(!runtime->GetClassLinker()->EnsureInitialized(soa.Self(), exception_class, true,
                                                             true))) {
