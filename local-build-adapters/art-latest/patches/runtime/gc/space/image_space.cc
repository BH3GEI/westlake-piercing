...
class ImageSpace::PatchObjectVisitor final {
 public:
  ...
  void VisitClass(ObjPtr<mirror::Class> klass, ObjPtr<mirror::Class> class_class)
    ...
    // Tolerate corrupt class references from erroneous classes in the boot image.
    if (klass == nullptr || class_class == nullptr ||
        reinterpret_cast<uintptr_t>(class_class.Ptr()) < 0x10000) {
      return;  // Skip corrupt class objects
    }
 ...
 public:
  ...
  static bool CheckImageComponentCount(const ImageSpace& space,
    ...
    if (header.GetComponentCount() != expected_component_count) {
      // PATCH: Tolerate component count mismatch in standalone mode.
      // Our boot image covers only core JARs but BCP has more (framework, MCD, etc.)
      LOG(WARNING) << "Component count mismatch in " << space.GetImageFilename()
                   << " (image=" << header.GetComponentCount()
                   << " expected=" << expected_component_count << ") — tolerating";
    }
    return true;
  }
  ...
  static MemMap LoadImageFile(const char* image_filename,
    ...
    if (!is_compressed && allow_direct_mapping) {
      ...
      if (map.IsValid()) {
        ...
        fflush(stderr);
        // Entry points fixed AFTER MaybeRelocateSpaces via VisitPackedArtMethods
...
bool ImageSpace::BootImageLayout::VerifyImageLocation(
  ...
  // PATCH: Skip path validation — allow relative BCP paths for standalone builds.
  ...
  for (const std::string& bcp_component : boot_class_path_) {
    if (bcp_component.empty()) {
      *error_msg = StringPrintf("Empty boot class path component");
      return false;
    }
  }
...
class ImageSpace::BootImageLoader {
 ...
 private:
  bool LoadImage(
    ...
    // PATCH: Reset ALL ArtMethod entry points in boot image to interpreter bridges.
    // The boot image was generated by dex2oat which bakes in entry points to its own
    // assembly stubs. These addresses don't exist in dalvikvm/libwestlake_art.so.
    // Walk every method and reset to our runtime's bridges.
    {
      // runtime_asm_entrypoints.h provides these via extern "C" declarations at file scope
      ...
      for (auto& space : spaces) {
        ...
        // Fix image_methods_ (resolution trampoline, imt conflict, etc.)
        ImageHeader* mutable_header = const_cast<ImageHeader*>(&header);
        for (size_t i = 0; i < static_cast<size_t>(ImageHeader::kImageMethodsCount); i++) {
          ArtMethod* m = mutable_header->GetImageMethod(static_cast<ImageHeader::ImageMethod>(i));
          if (m != nullptr) {
            m->SetEntryPointFromQuickCompiledCode(resolution);
            space_fixed++;
          }
        }

        // Fix ALL packed ArtMethods in the image
        ...
        if (native_cleared > 0) {
          fprintf(stderr, "[IMG] Cleared %d stale native JNI entry points\n", native_cleared);
          fflush(stderr);
        }

        total_fixed += space_fixed;
      }
      fprintf(stderr, "[IMG] Fixed %d method entry points across %zu image spaces\n",
              total_fixed, spaces.size());
      fflush(stderr);
    }
    ...
    for (const std::unique_ptr<ImageSpace>& space : spaces) {
      ...
      if (image_header.GetClassTableSection().Size() != 0u) {
        ...
        for (ClassTable::TableSlot& slot : temp_set) {
          slot.VisitRoot(class_table_visitor);
          ObjPtr<mirror::Class> klass = slot.Read<kWithoutReadBarrier>();
          if (klass == nullptr || reinterpret_cast<uintptr_t>(klass.Ptr()) < 0x10000) {
            continue;  // Skip corrupt class table entry
          }
          if (patched_objects->Test(klass.Ptr())) {
            continue;
          }
          patched_objects->Set(klass.Ptr());
          if (class_class == nullptr || reinterpret_cast<uintptr_t>(class_class.Ptr()) < 0x10000) {
            fprintf(stderr, "[IMG] CORRUPT class_class=%p, skipping VisitClass for klass=%p\n",
                    class_class.Ptr(), klass.Ptr());
            fflush(stderr);
            continue;
          }
    ...
    for (const std::unique_ptr<ImageSpace>& space : spaces) {
      ...
      for (uint32_t pos = sizeof(ImageHeader); pos != objects_end; ) {
        ...
        if (!patched_objects->Test(object)) {
          ...
          // Tolerate corrupt objects from erroneous classes: skip if class pointer
          // is out of range (not in the image or heap).
          if (klass == nullptr || reinterpret_cast<uintptr_t>(klass.Ptr()) < 0x10000) {
            // Skip this object — truly corrupt class pointer
            pos += kObjectAlignment;
            continue;
          }
          // Note: klass may already be relocated to dest by previous passes.
          // Don't check InSource — accept both source and dest addresses.
          main_patch_object_visitor.VisitObject(object);
          klass = object->GetClass<kVerifyNone, kWithoutReadBarrier>();
          // Patch DexCache native arrays (resolved methods/fields/types/strings)
          if (object->IsDexCache<kVerifyNone, kWithoutReadBarrier>()) {
            main_patch_object_visitor.VisitDexCacheArrays(
    ...
    std::unique_ptr<OatFile> oat_file;
    {
      ...
      if (component_count == 0u) {
        ...
        if (expected_boot_class_path != oat_boot_class_path) {
          // PATCH: Skip BCP path mismatch in standalone mode (relative vs absolute paths)
          LOG(WARNING) << "BCP mismatch (tolerating): image=" << oat_boot_class_path
                       << " expected=" << expected_boot_class_path;
        }
      } else {
        std::string local_error_msg;
...
bool ImageSpace::ValidateOatFile(const OatFile& oat_file,
  ...
  // PATCH: Skip read barrier validation — our runtime uses CMS (no read barriers)
  // but the phone's dex2oat generates with CC (read barriers enabled).
  // The pre-initialized class data is still valid; we just won't use read barriers at runtime.
  if (false && oat_file.GetOatHeader().GetKeyValueStoreSize() != 0 &&
