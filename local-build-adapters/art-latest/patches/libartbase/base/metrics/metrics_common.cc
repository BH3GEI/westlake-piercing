...
// XmlFormatter methods stubbed out -- they require tinyxml2 InsertNewChildElement
// which is not available in the A11 tinyxml2 library. These are never called in
// standalone dex2oat boot image creation.
void XmlFormatter::FormatBeginReport(uint64_t, const std::optional<SessionData>&) {}
void XmlFormatter::FormatReportCounter(DatumId, uint64_t) {}
void XmlFormatter::FormatReportHistogram(DatumId, int64_t, int64_t, const std::vector<uint32_t>&) {}
void XmlFormatter::FormatEndReport() {}
std::string XmlFormatter::GetAndResetBuffer() { return ""; }
