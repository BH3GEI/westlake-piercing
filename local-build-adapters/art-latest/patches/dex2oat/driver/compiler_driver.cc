...
class InitializeClassVisitor : public CompilationVisitor {
 public:
  ...
  void TryInitializeClass(Thread* self,
    ...
    const dex::TypeId& class_type_id = dex_file.GetTypeId(class_def->class_idx_);
    const char* descriptor = dex_file.GetStringData(class_type_id.descriptor_idx_);
    // [DAYU600-ARM64-SKIP-INIT] These boot classes' <clinit> hit an arm64 interpreter
    // dispatch bug (garbage receiver -> FindMethodToCall SIGSEGV) during AOT pre-init.
    // They are NOT VarHandle-critical (VarHandle is fixed by image_writer FieldVarHandle
    // fixup, not clinit), so skip pre-initializing them; they init lazily at runtime.
    {
      static const char* const kDayu600SkipInit[] = {
        "Ljava/lang/StackStreamFactory;",
        "Ljava/lang/StackStreamFactory$AbstractStackWalker;",
        // [DAYU600] finalizable-object-in-transaction abort -> GC -> use-after-free crash
        "Lorg/apache/harmony/xml/ExpatParser;",
        "Lorg/apache/harmony/xml/ExpatParser$ClonedAttributes;",
        nullptr,
      };
      for (int _i = 0; kDayu600SkipInit[_i] != nullptr; ++_i) {
        if (strcmp(descriptor, kDayu600SkipInit[_i]) == 0) {
          LOG(WARNING) << "[DAYU600-SKIP-INIT] " << descriptor;
          return;
        }
      }
    }
...
static void CompileDexFile(CompilerDriver* driver,
  ...
  auto compile = [&context, &compile_fn, profile_index](size_t class_def_index) {
    ...
    if (klass == nullptr) {
      ...
      // Skip a duplicate class (as the resolved class is from another, earlier dex file).
      return;  // Do not update state.
    } else if (klass->IsErroneous()) {
      // Skip compilation of methods from erroneous classes.
      // Their DexCache or resolved types may be null, causing SIGSEGV in the compiler.
      return;
    } else {
