...
class OatWriter::WriteCodeMethodVisitor : public OrderedMethodVisitor {
 ...
 private:
  ...
  ObjPtr<mirror::Class> GetTargetType(const LinkerPatch& patch)
    ...
    // Null type is possible when compiled methods reference unresolved/erroneous classes.
    // Caller must handle gracefully (zero-patch to fall back to interpreter at runtime).
    return type;
  }
