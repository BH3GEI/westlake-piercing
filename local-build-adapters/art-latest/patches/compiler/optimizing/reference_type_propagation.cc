...
ReferenceTypeInfo ReferenceTypePropagation::MergeTypes(const ReferenceTypeInfo& a,
  ...
  if (a.GetTypeHandle().Get() == b.GetTypeHandle().Get()) {
    ...
    ObjPtr<mirror::Class> common = a_type_handle->GetCommonSuperClass(b_type_handle);
    // In standalone boot image builds, erroneous classes may have incomplete
    // hierarchies causing GetCommonSuperClass to return nullptr. Fall back to Object.
    if (UNLIKELY(common == nullptr)) {
      result_type_handle = handle_cache->GetObjectClassHandle();
    } else {
      result_type_handle = handle_cache->NewHandle(common);
    }
    is_exact = false;
  } else {
    // This can happen if:
