 ...
 * arm64 / aarch64-linux-ohos / musl (ported from arm32: 64-bit hexcat, aarch64 syscall nrs).
 *
...
/* ---- raw syscall (catch exit_group / exit issued directly; aarch64: 94/93) ---- */
...
long syscall(long num, ...) {
    ...
    va_end(ap);
    if (num == SYS_exit_group || num == SYS_exit)
