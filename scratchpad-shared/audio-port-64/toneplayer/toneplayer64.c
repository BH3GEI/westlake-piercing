 ...
 * Plays a 440Hz stereo sine for ~5s. arm64 / dlopen libohaudio (lib64, lib32 fallback). */
#include <dlfcn.h>
#include <stdio.h>
#include <stdint.h>
#include <string.h>
...
int main(void) {
    void* h = dlopen("/system/lib64/ndk/libohaudio.so", RTLD_NOW);
    if (!h) {
        printf("dlopen lib64 fail: %s -- trying 32-bit path\n", dlerror());
        h = dlopen("/system/lib/ndk/libohaudio.so", RTLD_NOW);
    }
