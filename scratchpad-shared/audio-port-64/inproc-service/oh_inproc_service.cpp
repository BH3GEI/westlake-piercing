#include <cstring>
/*
 * oh_inproc_service.cpp — arm64/OHOS-6.1 port (GATE 1, 2026-07-08)
 *
 * 32->64 ABI AUDIT RESULT: source is 64-bit clean as written; no functional
 * changes were required. Specifically:
 *   - All native handles are `jobject` JNI global refs (pointer-width by
 *     construction on both ABIs); NO native pointer is ever stored in a
 *     jint/jlong Java field and there is NO jlong<->handle round-trip.
 *   - connId is plain int end-to-end (matches jint = 32 bits on both ABIs);
 *     the one Java-bound crossing casts explicitly: `(jint)connId`.
 *   - nowMs() uses explicit int64_t arithmetic; tv_sec is cast to int64_t
 *     before the *1000, so 64-bit time_t on aarch64 musl is handled.
 *   - %p log sites cast through (void*).
 *   - JNI varargs calls audited for AAPCS64: every integer literal lands in
 *     an int slot of the signature, never in a jobject slot.
 * Compiled with: OHOS clang++ --target=aarch64-linux-ohos (musl sysroot).
 *
...
// ---------------------------------------------------------------------------
// JNI registration: override adapter.client.ActivityManagerAdapter natives so
// in-app Service binds are handled in-process. Non-Service abilities return -1
// and fall back to the OHOS path exactly as the unpatched bridge did.
// ---------------------------------------------------------------------------
static jint JNICALL nativeConnectAbility(JNIEnv* env, jclass, jstring jbundle, jstring jability, jint connId) {
    const char* bundle = jbundle ? env->GetStringUTFChars(jbundle, nullptr) : nullptr;
    const char* ability = jability ? env->GetStringUTFChars(jability, nullptr) : nullptr;
    jint r = -1;
    if (ability && inproc_isService(env, ability)) {
        r = inproc_bindService2(env, bundle, ability, connId);
    }
    if (jbundle && bundle) env->ReleaseStringUTFChars(jbundle, bundle);
    if (jability && ability) env->ReleaseStringUTFChars(jability, ability);
    return r;
}

static jint JNICALL nativeDisconnectAbility(JNIEnv*, jclass, jint connId) {
    return inproc_disconnect(nullptr, connId);
}

static const JNINativeMethod g_amMethods[] = {
    {"nativeConnectAbility",    "(Ljava/lang/String;Ljava/lang/String;I)I", (void*)nativeConnectAbility},
    {"nativeDisconnectAbility", "(I)I",                                      (void*)nativeDisconnectAbility},
};

extern "C" JNIEXPORT jint JNICALL JNI_OnLoad(JavaVM* vm, void*) {
    JNIEnv* env = nullptr;
    if (vm->GetEnv((void**)&env, JNI_VERSION_1_6) != JNI_OK || !env) {
        IPERR("JNI_OnLoad: GetEnv failed");
        return JNI_VERSION_1_6;
    }
    if (!g_vm) g_vm = vm;
    jclass cls = env->FindClass("adapter/client/ActivityManagerAdapter");
    if (!cls) {
        if (env->ExceptionCheck()) env->ExceptionClear();
        IPERR("JNI_OnLoad: adapter/client/ActivityManagerAdapter not found");
        return JNI_VERSION_1_6;
    }
    if (env->RegisterNatives(cls, g_amMethods, 2) != 0) {
        IPERR("JNI_OnLoad: RegisterNatives failed");
        if (env->ExceptionCheck()) env->ExceptionClear();
    } else {
        IPLOG("JNI_OnLoad: registered in-proc bind/disconnect on ActivityManagerAdapter");
    }
