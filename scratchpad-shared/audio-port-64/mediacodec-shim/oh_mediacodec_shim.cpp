// oh_mediacodec_shim.cpp  [FIX-AUDIO 2026-07-02] [ARM64 PORT 2026-07-08]
// 64-bit port notes: mNativeContext handle stays jlong ("J"); all native-pointer
// <-> Java crossings use (jlong)(uintptr_t)ptr / (T*)(uintptr_t)handle. dlopen
// fallback prefers /system/lib64/ndk (arm64 dir) before /system/lib/ndk.
...
static void resolveOH() {
    ...
    if (!a) a = dlopen("/system/lib64/ndk/libnative_media_acodec.so", RTLD_NOW);   // arm64 NDK dir
    if (!a) a = dlopen("/system/lib64/libnative_media_acodec.so", RTLD_NOW);
    if (!a) a = dlopen("/system/lib/ndk/libnative_media_acodec.so", RTLD_NOW);     // 32-bit last resort
    void* c = dlopen("libnative_media_core.so", RTLD_NOW);
    if (!c) c = dlopen("/system/lib64/ndk/libnative_media_core.so", RTLD_NOW);     // arm64 NDK dir
    if (!c) c = dlopen("/system/lib64/libnative_media_core.so", RTLD_NOW);
    if (!c) c = dlopen("/system/lib/ndk/libnative_media_core.so", RTLD_NOW);       // 32-bit last resort
...
static Codec* getCodec(JNIEnv* env, jobject thiz) {
    ...
    return (Codec*)(uintptr_t)env->GetLongField(thiz, f_ctx);   // 64-bit: jlong -> uintptr_t -> ptr, no truncation
}
...
static void nSetup(JNIEnv* env, jobject thiz, jstring jname, jboolean nameIsType, jboolean encoder, jint, jint) {
    ...
    if (f_ctx) env->SetLongField(thiz, f_ctx, (jlong)(uintptr_t)c);   // 64-bit: full pointer preserved in jlong
}
...
static void nRelease(JNIEnv* env, jobject thiz) { Codec* c=getCodec(env,thiz); if(c){ if(c->codec&&g_ok) p_Destroy(c->codec); if(f_ctx) env->SetLongField(thiz,f_ctx,(jlong)0); delete c; } }
