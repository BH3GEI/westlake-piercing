...
2. **WMS 真窗口(XL)+ sceneboard 生死问【M5 scoping 完成,已锐化】**:调用面全摸清(SAMGR→GetSystemAbility(4606)→iface_cast IWindowManager→CreateWindow/AddWindow);需 4 shim(node-type RSSurfaceNode::Create+PaddedConfig+64/2-arg Create/WindowProperty::SetWindowName/SetBufferAvailableCallback)。**MAKE-OR-BREAK=sceneboard**:origin 跑 sceneboard=false,uis7885 出厂几乎必 ON,SA 4606 可能只服务 scene 路(CreateAndConnectSpecificSession)不解 legacy CreateWindow→须重写 ISession/ISessionStage(工作量差一个数量级);**仅板上读 hilog OH_WindowMgrClient 可判**。`libwms.z.so` 板上已证存在(1.28MB,120 WindowManagerProxy/WindowProperty 符号,observed),link 目标在;是否导出 broker 待 llvm-readelf 细验。
