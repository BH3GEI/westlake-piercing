#!/usr/bin/env bash
# Build libwestlake_upscreen_renderer.so — in-process JNI bridge that renders the
...
OBJ_DIR="$OUT_DIR/obj"
OUT="$OUT_DIR/libwestlake_upscreen_renderer.so"
...
echo "== compiling westlake_upscreen_renderer.cpp =="
"$CLANGXX" "${HWUI_FLAGS[@]}" "${HWUI_INCLUDES[@]}" \
    -c "$HERE/westlake_upscreen_renderer.cpp" -o "$OBJ_DIR/westlake_upscreen_renderer.o"
...
"$CLANGXX" --target=aarch64-linux-ohos --sysroot="$SYSROOT" -shared -fPIC -static-libstdc++ \
    ...
    -Wl,-soname,libwestlake_upscreen_renderer.so \
    "$OBJ_DIR/westlake_upscreen_renderer.o" \
