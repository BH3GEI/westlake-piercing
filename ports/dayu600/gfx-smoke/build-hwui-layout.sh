#!/usr/bin/env bash
# Build hwui_layout for DAYU600 (aarch64-linux-ohos).
#
...
# hwui_layout.cpp. The interposers (egl/skia/oh_anativewindow/ohos_display_surface/
...
# ---------------------------------------------------------------------------
echo "== [C] compiling hwui_layout TU =="
...
"$CLANGXX" "${HWUI_FLAGS[@]}" "${HWUI_INCLUDES[@]}" \
    -c "$HERE/hwui_layout.cpp" -o "$OBJ_DIR/hwui_layout.o"
