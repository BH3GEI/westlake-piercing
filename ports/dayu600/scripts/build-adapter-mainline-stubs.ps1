...
$src = Join-Path $port 'adapter-mainline-stubs-src'
$out = Join-Path $port 'out\adapter-mainline-stubs'
...
$javacArgs = @('--release', '8', '-XDignore.symbol.file', '-cp', $androidJar, '-d', $classes) + $sources
& $javac @javacArgs
if ($LASTEXITCODE -ne 0) { throw "javac failed with $LASTEXITCODE" }

Push-Location $port
try {
    $classFiles = @(Get-ChildItem -Path 'out\adapter-mainline-stubs\classes' -Recurse -Filter '*.class' |
        ForEach-Object { $_.FullName.Substring($port.Path.Length + 1) })
    & $d8 --lib $androidJar --min-api 24 --output 'out\adapter-mainline-stubs\dex' @classFiles
    if ($LASTEXITCODE -ne 0) { throw "d8 failed with $LASTEXITCODE" }

    Push-Location (Join-Path $out 'dex')
    try {
        & $jar cf (Join-Path $out 'adapter-mainline-stubs.jar') 'classes.dex'
...
Get-Item (Join-Path $out 'adapter-mainline-stubs.jar') | Select-Object FullName,Length
