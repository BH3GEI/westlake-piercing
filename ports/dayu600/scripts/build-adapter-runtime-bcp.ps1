...
$src = Join-Path $port 'adapter-runtime-src'
$out = Join-Path $port 'out\adapter-runtime'
...
Push-Location $port
try {
    $classFiles = @(Get-ChildItem -Path 'out\adapter-runtime\classes' -Recurse -Filter '*.class' |
        ForEach-Object { $_.FullName.Substring($port.Path.Length + 1) })
    & $d8 --lib $androidJar --min-api 24 --output 'out\adapter-runtime\dex' @classFiles
    if ($LASTEXITCODE -ne 0) { throw "d8 failed with $LASTEXITCODE" }

    Push-Location (Join-Path $out 'dex')
    try {
        & $jar cf (Join-Path $out 'adapter-runtime-bcp.jar') 'classes.dex'
...
Get-Item (Join-Path $out 'adapter-runtime-bcp.jar') | Select-Object FullName,Length
